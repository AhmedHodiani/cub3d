...
/*   Updated: 2025/11/23 22:11:10 by ataher           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cast_single_ray(t_game *game, double ray_dir_x, double ray_dir_y)
{
	...
	screen_x = MINIMAP_OFFSET_X + (int)(ray_x * MINIMAP_SCALE);
	screen_y = MINIMAP_OFFSET_Y + (int)(ray_y * MINIMAP_SCALE);
	draw_line(game,
		MINIMAP_OFFSET_X + (int)(game->player.x * MINIMAP_SCALE),
		MINIMAP_OFFSET_Y + (int)(game->player.y * MINIMAP_SCALE),
		screen_x, screen_y);
}
